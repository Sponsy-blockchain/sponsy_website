	...
	wHTML.prototype.lineStep = function (controller) {
		...
		function constructorLine() {
			...
			var stepItem2Position = $('.step2').position().top,
			    stepItem3Position = $('.step3').position().top + $('.step3__img').position().top + $('.step3__img').height();

			$lineWrap.height(lineWrapHeight + stepItemHeight);

			$('.step__item--2').css('top', stepItem2Position + 20);
			$('.step__item--3').css('top', stepItem3Position - stepItemHeight);
		}
		}
...
jQuery(document).ready(function ($) {
	...
	$('.interview__img').click(function() {
		...
		var video_label = Number(inteview_id) == 5 ? 'SponsyApp_Video' : Number(inteview_id) == 6 ? 'SponsyIntro_Video' : "inter_"+interview_id;
